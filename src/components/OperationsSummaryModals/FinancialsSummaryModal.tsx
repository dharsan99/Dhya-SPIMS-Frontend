...
interface FinancialsSummaryModalProps {
  open: boolean;
  onClose: () => void;
  data: {
    totalRevenue: number;
...
const FinancialsSummaryModal: React.FC<FinancialsSummaryModalProps> = ({ open, onClose, data }) => {
  if (!open) return null;
  return (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-green-50 p-3 rounded-lg">
                ...
                <div className="text-xl font-semibold">₹{data.totalRevenue}</div>
                <div className="text-sm text-green-600">+{data.revenueTrend}%</div>
              </div>
              <div className="bg-red-50 p-3 rounded-lg">
                <div className="text-sm text-red-700">Total Expenses</div>
                <div className="text-xl font-semibold">₹{data.totalExpenses}</div>
                <div className="text-sm text-red-600">+{data.expensesTrend}%</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-50 p-3 rounded-lg">
                <div className="text-sm text-blue-700">Net Profit</div>
                <div className="text-xl font-semibold">₹{data.netProfit}</div>
              </div>
              <div className="bg-purple-50 p-3 rounded-lg">
                <div className="text-sm text-purple-700">Profit Margin</div>
                <div className="text-xl font-semibold">{data.profitMargin}%</div>
              </div>
            </div>
          </div>
        </div>
...
export default FinancialsSummaryModal; 