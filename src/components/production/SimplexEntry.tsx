...
interface SimplexEntryProps {
  defaultOrderId: string;
  availableOrders: any[];
}

const SimplexEntry: React.FC<SimplexEntryProps> = ({ defaultOrderId, availableOrders }) => {
  const { simplex, setSimplex } = useProductionStore();
  ...
  useEffect(() => {
    if (defaultOrderId) {
      const initializedSimplex = simplex.map(machine => {
  ...
  const handleBlur = (machineIdx: number, shift: number) => {
    ...
    const value = Number(simplex[machineIdx][`shift${shift}` as keyof MachineShiftEntry]);
  ...
  const isValid = (machineIdx: number, shift: number) => {
    const machine = simplex[machineIdx];
  ...
  return (
    <div className="space-y-6">
      {simplex.map((machine, machineIdx) => (
      ...
      <div className="mt-4 font-medium text-gray-800 text-base">
        Total Today: {simplex.reduce((sum, machine) => sum + getMachineTotal(simplex.indexOf(machine)), 0)} kgs
      </div>
    </div>
  );
};

export default SimplexEntry; 