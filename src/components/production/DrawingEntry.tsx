...
interface DrawingEntryProps {
  defaultOrderId: string;
  availableOrders: any[];
}

const DrawingEntry: React.FC<DrawingEntryProps> = ({ defaultOrderId, availableOrders }) => {
  const { drawing, setDrawing } = useProductionStore();
  ...
  useEffect(() => {
    if (defaultOrderId) {
      const initializedDrawing = drawing.map(machine => {
  ...
  const handleBlur = (machineIdx: number, shift: number) => {
    ...
    const value = Number(drawing[machineIdx][`shift${shift}` as keyof MachineShiftEntry]);
  ...
  const isValid = (machineIdx: number, shift: number) => {
    const machine = drawing[machineIdx];
  ...
  return (
    <div className="space-y-6">
      {drawing.map((machine, machineIdx) => (
      ...
      <div className="mt-4 font-medium text-gray-800 text-base">
        Total Today: {drawing.reduce((sum, machine) => sum + getMachineTotal(drawing.indexOf(machine)), 0)} kgs
      </div>
    </div>
  );
};
